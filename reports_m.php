...
<input type="submit" class="uk-button uk-button-default" type="submit" name="search" value="بحث"/>
      ...
      <h3>  <?php echo '<a href="reports.php?i='.$_GET['i'].'"> تقارير  </a>'; ?> </h3>
...
$arr = myda($aee);
        ...
        while($nnswuww<count($arr)){
            ...
            $s835wa = mysqli_query($mu2,"SELECT * FROM el2st WHERE date_today LIKE '".myf1($arr[$nnswuww])."'");
             ...
             $m8wmu_u = mysqli_fetch_assoc($s835wa_u);
                 ...
                 if(!empty($m8wmu_u['id'])){
                     if($m8wmu['kind'] == null){
                         $el2st+=myf2($m8wmu["total"]);
                     }else if($m8wmu['kind']==myf1("cash")){
                         $cash+=myf2($m8wmu["total"]);
                     }
                 }
        ...
        echo '
         <div class="uk-margin">
    <b> الاقساط :</b><span>'.$el2st.'</span>
    <b>الكاش :</b><span>'.$cash.'</span>
...
$nnswuww = 0;
        ...
        while($nnswuww<count($arr)){
          ...
          $s835wa = mysqli_query($mu2,"SELECT * FROM el2st WHERE date_today LIKE '".myf1($arr[$nnswuww])."'");
             ...
             $m8wmu_u = mysqli_fetch_assoc($s835wa_u);
                 ...
                 if(!empty($m8wmu_u['id'])){
                   ...
                   $tex="قسط";
                            $nmnus++;
                          $s835wa_N = mysqli_query($mu2,"SELECT * FROM el2st WHERE date_today LIKE '".myf1($arr[$nnswuww])."' AND user_id='".myf1($m8wmu_u['id'])."' AND notes!='' AND kind=''");
            
            $m8wmu_n = mysqli_fetch_assoc($s835wa_N);
                                                echo '
          ...
          <tr class="table__row">
            ...
            <td><a href="clientpage.php?i='.myf1($m8wmu_u['id']).'">'.myf2($m8wmu_u['name']).'</a></td>
            <td>'.myf2($m8wmu['price']).'</td>
            <td>'.myf2($m8wmu['useful']).'%</td>
            <td>'.myf2($m8wmu['total']).'</td>
            <td> '.myf2($m8wmu_n['notes']).' </td>
            <td> '.$tex.' </td>
            ...
            ';
                         ...
                         $tex="كاش";
                            $nmnus++;
                           $s835wa_N = mysqli_query($mu2,"SELECT * FROM el2st WHERE date_today LIKE '".myf1($arr[$nnswuww])."' AND user_id='".$m8wmu['user_id']."' AND notes!='' AND kind='".myf1("cash")."'");
     ...
     $s835wa = mysqli_query($mu2,"SELECT * FROM el2st WHERE date_today LIKE '".myf1($date_today)."'");
        ...
        $nmnus=0;
          ...
          $s835wa = mysqli_query($mu2,"SELECT * FROM el2st WHERE date_today LIKE '".myf1($date_today)."'");
             ...
             $m8wmu_u = mysqli_fetch_assoc($s835wa_u);
                 ...
                 if(!empty($m8wmu_u['id'])){
                   ...
                   $tex="قسط";
                            $nmnus++;
                          $s835wa_N = mysqli_query($mu2,"SELECT * FROM el2st WHERE date_today LIKE '".myf1($date_today)."' AND user_id='".myf1($m8wmu_u['id'])."' AND notes!='' AND kind=''");
          ...
          <tr class="table__row">
            ...
            ';
                         ...
                         $tex="كاش";
                            $nmnus++;
                           $s835wa_N = mysqli_query($mu2,"SELECT * FROM el2st WHERE date_today LIKE '".myf1($date_today)."' AND user_id='".$m8wmu['user_id']."' AND notes!='' AND kind='".myf1("cash")."'");
